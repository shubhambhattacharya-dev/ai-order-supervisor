...
async def final_output(order_id: str, events: list, actions: list) -> dict:
    ...
    try:
        ...
        _persist_report(order_id, report.get("summary", ""), actions, report.get("key_learnings", []), report.get("feedback", []))
        return report
        ...
        fallback_report = {
            ...
            "key_learnings": [
                "Automated order lifecycle supervision executed successfully.",
                "Policy allowlists prevented unauthorized action execution.",
            ],
            "feedback": [
                "Supervisor operated reliably across event signals.",
            ],
        }
        _persist_report(order_id, fallback_report["summary"], actions, fallback_report["key_learnings"], fallback_report["feedback"])
        return fallback_report


def _persist_report(order_id: str, summary: str, actions: list, learnings: list, feedback: list) -> None:
    try:
        import os
        import psycopg
        ...
        if activity.in_activity():
            activity.logger.warning("final output persist skipped: %s", exc)